...
Render (`render.yaml`): the web app is a Render Static Site with a rewrite of all paths to `/index.html` for client-side routing — this rewrite must stay scoped to the static site only. The API is a separate Render service; `/api/v1/health` and `/api/v1/health/readiness` are Express-handled, not part of the SPA rewrite. `docs/phase-07-launch-runbook.md` covers the launch/deploy procedure; other `docs/phase-*.md` files are dated QA/remediation snapshots from earlier delivery phases.
