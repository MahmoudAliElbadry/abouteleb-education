...
- When a stored user "but unverified" try to log in .. when the un verified error appear also show a verify account button .. that redirect him the verify account page .. and automatically send him a verification OTP in his mail.
...
- Increase the red paragraph font size in the top of each section, like this:
  [<p>تعرف علينا</p> in PublicPage (at /src/App.tsx) in App (at /src/App.tsx) selector: #who-we-are]

- The arrow is not in the center of the button:
  [<button aria-label="تشغيل الفيديو التعريفي" type="button" /> in PublicPage (at /src/App.tsx) in App (at /src/App.tsx) selector: [aria-label="تشغيل الفيديو التعريفي"]]
